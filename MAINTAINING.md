...
clawhub package publish "$PWD" --family code-plugin --version "$VERSION" --source-repo "$SOURCE_REPO" --source-commit "$COMMIT" --source-ref "v$VERSION" --dry-run
clawhub package publish "$PWD" --family code-plugin --version "$VERSION" --source-repo "$SOURCE_REPO" --source-commit "$COMMIT" --source-ref "v$VERSION"
```

Both tasks read the version from `package.json` and publish with
`--source-ref v<version>`, `--source-repo` derived from `origin`, and the tag's
commit SHA. They depend on `Release: Verify Version Tag`, which fails if the
matching version tag does not exist locally and on `origin`.
